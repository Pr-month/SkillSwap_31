...
export class SkillsService {
  ...
  async create(createSkillDto: CreateSkillDto, owner: User): Promise<Skill> {
    ...
    if (!category) {
      throw new NotFoundException(`Категория с ID "${createSkillDto.categoryId}" не найдена.`);
    }
  ...
  async update(id: string, updateSkillDto: UpdateSkillDto, userId: string): Promise<Skill> {
    ...
    if (updateSkillDto.categoryId !== undefined) {
      ...
      if (!category) {
        throw new NotFoundException(`Категория с ID "${updateSkillDto.categoryId}" не найдена.`);
      }
    }
